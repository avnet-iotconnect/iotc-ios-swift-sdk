...
//        print("connectMQTTAgain: ", boolIsInternetAvailableYN)
    ...
    func disconnect() {
        ...
        if mqtt != nil {
            mqtt!.disconnect()
            // mqtt = nil
            objCommon.manageDebugLog(code: Log.Info.INFO_IN03.rawValue, uniqueId: strUniqueID, cpId: strCPID, message: "", logFlag: true, isDebugEnabled: boolDebugYN)
...
extension MQTTClient: CocoaMQTTDelegate {
    ...
    func mqtt(_ mqtt: CocoaMQTT, didConnectAck ack: CocoaMQTTConnAck) {
        ...
        if ack == .accept {
            ...
            IoTConnectManager.sharedInstance.twinPropertyPubTopic = set?["pub"] as! String
            mqtt.subscribe(set?["pub"] as! String, qos: .qos1)
            print("twinResponseSubTopic \(set?["sub"] as! String)")
            IoTConnectManager.sharedInstance.twinPropertySubTopic = set?["sub"] as! String
            mqtt.subscribe(set?["sub"] as! String, qos: .qos1)
            IoTConnectManager.sharedInstance.twinResponseSubTopic = set?["subForAll"] as! String
            IoTConnectManager.sharedInstance.twinResponsePubTopic = set?["pubForAll"] as! String
#else
            mqtt.subscribe(IoTConnectManager.sharedInstance.twinPropertySubTopic, qos: .qos1)
            mqtt.subscribe(IoTConnectManager.sharedInstance.twinPropertySubTopic, qos: .qos1)
            print("twinResponseSubTopic \(IoTConnectManager.sharedInstance.twinResponseSubTopic)")
